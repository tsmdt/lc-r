...
This same exact data can be visualized in a couple different ways by replacing the geom\_bar() function with either `geom_density()` (adding a logarithmic x scale) or `geom_freqpoly()`:
...
- Add color to the `geom_jitter` argument on your violin plot according to the item `subCollection`. *Hint:* If you get the error `object 'subCollection' not found` or `invalid color name 'subCollection'` then consider `color` as an aesthetic mapping.
